class AssessmentsController < ApplicationController
  ...
  # Do Assessment
  def edit
    Rails.logger.unknown("User requested to start assessment on contact ID: #{@contact.id}")
  ...
  def update
    ...
    Rails.logger.unknown("User updated assessment ID #{@assessment.id} on contact ID: #{@contact.id}")
  ...
  def new
    Rails.logger.unknown('User requested page: new assessment')
  ...
  def create
    ...
    Rails.logger.unknown("User created a new assessment on contact ID: #{@contact.id}")
  end

  def start; end

  def fail
    Rails.logger.unknown("User requested fail assignment page for assessment ID: #{params[:id]}")
  ...
  def assign
    Rails.logger.unknown("User requested update assignment page for assessment ID: #{params[:id]}")
    @assigned_to_options = construct_assigned_to_options(true)
    @assignment_form = AssessmentAssignmentForm.from_id(params[:id])
  end

  def update_assignment
    Rails.logger.unknown("User updated assignments for assessment ID: #{params[:id]}")
  ...
  # Complete
  def complete
    Rails.logger.unknown("User request page complete assessment on contact ID: #{@contact.id}")
  ...
  def update_completion
    ...
    @completion_form.existing_check_in = Need.where(contact_id: @contact.id, category: 'Check in', status: Need.statuses[:to_do]).where.not(id: @assessment.id).first
    @completion_form.existing_mdt_review = Need.where(contact_id: @contact.id, category: 'mdt review', status: Need.statuses[:to_do]).first
    Rails.logger.unknown("User completed assessment on contact ID: #{@contact.id}")
