class NeedsAssigneeNotifier
  def self.notify_new_assignee(need)
    return unless need.saved_change_to_user_id? || need.saved_change_to_role_id? || need.assessment_id.present?
  ...
  def self.bulk_notify_new_assignee(needs)
    changed_needs = needs.select { |need| need.saved_change_to_user_id? || need.saved_change_to_role_id? || need.assessment_id.present? }
  ...
  def self.send_need_email(need, email_links)
    return unless need.send_email == true
